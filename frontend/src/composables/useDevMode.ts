/**
 * Dev Mode / Debug Console Composable
 * Port of public/js/debug-console.js to Vue 3 Composition API
 */
...
const logBuffer = ref<Array<{ timestamp: string; level: string; message: string; network?: any }>>([])
const isViewerOpen = ref(false)
const isButtonVisible = ref(false)
const devUnlockClickCount = ref(0)
...
function addLog(level: string, args: any[], meta: any = {}) {
  ...
  logBuffer.value.push({ timestamp, level, message, ...meta })
...
function interceptConsole() {
  ...
  const intercept = (type: 'log' | 'warn' | 'error' | 'info') => {
    return (...args: any[]) => {
      const fn = originalConsole[type]
      if (fn) fn.apply(console, args)
      const severity = type === 'error' ? 'ERROR' : type === 'warn' ? 'WARN' : type === 'info' ? 'INFO' : 'LOG'
      addLog(severity, args)
    }
  }
  ...
  window.fetch = async (...args: any[]) => {
    ...
    try {
      const res = await originalFetch.apply(null, args as [any, any?])
      ...
      const duration = Math.round(performance.now() - start)
      addLog('ERROR', [`${method} ${url} → FAILED (${duration}ms): ${err instanceof Error ? err.message : err}`], {
...
function createStyles() {
  ...
  style.textContent = `
    ...
    #debug-console-content .log-level {
      font-weight: bold;
      min-width: 50px;
      color: #94a3b8;
    }
...
function createUI() {
  ...
  if (!document.getElementById('debug-toggle-btn')) {
    ...
    const savedVisible = localStorage.getItem('drCatDebugConsoleVisible') === 'true'
    btn.style.display = savedVisible ? 'flex' : 'none'
    isButtonVisible.value = savedVisible
  ...
  if (!document.getElementById('debug-console-panel')) {
    ...
    document.getElementById('debug-copy-btn')?.addEventListener('click', () => {
      const text = logBuffer.value.map(l => `[${l.timestamp}] [${l.level}] ${l.message}`).join('\n')
...
function renderLogs() {
  ...
  container.innerHTML = logBuffer.value.map(log => {
...
function toggleViewer() {
  ...
  panel.style.display = isViewerOpen.value ? 'flex' : 'none'
  if (isViewerOpen.value) renderLogs()
}

function attemptDevUnlock() {
  if (devUnlockCooldown) return

  devUnlockClickCount.value++
  if (devUnlockClickCount.value >= DEV_UNLOCK_THRESHOLD) {
    devUnlockClickCount.value = 0
    devUnlockCooldown = true
    ;(window as any).__drCatDevDiagnosticsUnlocked = true
    localStorage.setItem('drCatDevDiagnosticsUnlocked', 'true')

    const appStore = useAppStore()
    appStore.showToast('🔓 Diagnostics/Performance déverrouillés !', 'fa-unlock', 4000)
    ...
    window.dispatchEvent(new CustomEvent('dr-cat-dev-unlocked'))
  ...
  if (logoClicks >= 10) {
    ...
    if (btn) {
      ...
      if (currentlyVisible) {
        ...
        ;(window as any).__drCatDevDiagnosticsUnlocked = false
        window.dispatchEvent(new CustomEvent('dr-cat-dev-unlocked'))
        const appStore = useAppStore()
        appStore.showToast('🐛 Mode Débogage désactivé.', 'fa-bug', 3000)
      } else {
        btn.style.setProperty('display', 'flex', 'important')
        isButtonVisible.value = true
        localStorage.setItem('drCatDebugConsoleVisible', 'true')
        const appStore = useAppStore()
        appStore.showToast('🐛 Mode Débogage activé ! Tapotez 3 fois sur l\'icône 🐛 pour déverrouiller les diagnostics.', 'fa-bug', 5000)
      }
    }
  }
}

function registerLogoListeners() {
...
export function useDevMode() {
  onMounted(() => {
    ;(window as any).__drCatDevDiagnosticsUnlocked = localStorage.getItem('drCatDevDiagnosticsUnlocked') === 'true'
  ...
  return {
    ...
    exportLogs: () => {
      ...
      a.href = url
      a.download = `drcat-debug-logs-${new Date().toISOString().slice(0, 19).replace(/:/g, '-')}.txt`
      a.click()
