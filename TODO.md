...
- `[ ]` **Database Schema Validation**: Add a JSON schema validator that runs on server boot to verify `cats_db.json` structure. If a field is missing or has the wrong type (e.g., a manual edit typo), log a clear warning instead of crashing silently.
...
- `[ ]` **Admin URL Updater UI**: Add a form in the Admin Diagnostics panel to update the server URL in `remote_server_config.json` directly from the browser, triggering a hot-reload of the providers list without a server restart.
...
- `[ ]` **Test Suite CI Integration**: Run `node test_api.js` automatically as part of the GitHub Actions build to catch API regressions before the APK is compiled.
